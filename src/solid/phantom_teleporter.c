...
static void PhantomTeleporter_Update(struct Solid* p) {
  static const SolidFunc sUpdates[1] = {
      (SolidFunc)updatePhantomTeleporter,
  };
  ...
  switch ((p->s).mode[2]) {
    case 0: {
      if (!FLAG(gCurStory.s.gameflags, u16_ARRAY_08371064[(p->s).work[0]])) {
        SetMotion(&p->s, MOTION(SM123_TELEPORTAL, 1));
        INIT_BODY(p, sCollisions, 0, onCollision);
        p->player = NULL;
      } else {
        SetMotion(&p->s, MOTION(SM123_TELEPORTAL, 0));
        EXIT_BODY(p);
      }
      (p->s).mode[2]++;
      FALLTHROUGH;
    ...
    case 2: {
      ...
      SetMotion(&p->s, MOTION(SM123_TELEPORTAL, 2));
      (p->s).work[2] = 20;
      (p->s).mode[2]++;
      break;
    }
    case 3: {
      register struct Coord* dest asm("r1");
      UpdateMotionGraphic(&p->s);
      if ((p->s).work[2] != 0) {
        (p->s).work[2]--;
        break;
      }
      Camera_SetMode(&gStageRun.vm.camera, 1);
      dest = FUN_08019d20(&(p->s).coord);
      p->destination = dest;
      if (dest != NULL) {
    ...
    case 4: {
      ...
      SetMotion(&p->s, MOTION(SM123_TELEPORTAL, 3));
    ...
    case 5: {
      ...
      if (gStageRun.vm.transition == TRANSITION_NONE) {
        gStageRun.vm.unk_004 &= ~(1 << 1);
        Camera_SetMode(&gStageRun.vm.camera, 6);
