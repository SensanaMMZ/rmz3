...
With the ELF imported once, the whole reconstruction backlog goes through in a
single analysis pass:

```sh
python3 tools/classify_holdouts.py            # -> notes/holdouts-pure.md
python3 tools/ghidra/batch_decompile.py       # -> build/ghidra-drafts/*.c
python3 tools/ghidra/resolve_pool_refs.py     # resolve DAT_08xxxxxx placeholders
```

357 of 357 pure stubs decompiled with zero failures on the first run. The
Ghidra project is cached under `build/ghidra-proj`, so re-runs start
immediately instead of re-analysing 8 MB.

`resolve_pool_refs.py` exists because Ghidra names an unrecognised literal-pool
slot `DAT_08xxxxxx` and dereferences it, so a global access reads as
`*DAT_080ea274` -- 5,648 of them across the drafts. The slot is four bytes of
ROM: read it, resolve the value against the symbol map, and rename the token,
so it becomes `*p_gVideoRegBuffer`. When the slot holds a plain constant rather
than an address, the constant is substituted instead -- that is the value you
need in order to write the C. Tokens behind `*` or `&` are left alone, since
there Ghidra is treating the slot as an object and substituting would change
the meaning. 99.9% resolve.

Drafts are pseudo-C, not matches. The cross-check rule still holds: trust a
reconstruction when m2c and Ghidra agree on the structure, then match it with
the mdiff loop. `build/` is gitignored -- the drafts are regenerated by the
three commands above rather than committed.
